...
import (
	...
	"github.com/oldweipro/gin-admin/pkg/utils/request"
	"io"
	"strings"
...
func (autoApi *AutoCodeApi) GetDB(c *gin.Context) {
	...
	for _, db := range app.Config.DBList {
...
func (autoApi *AutoCodeApi) GetTables(c *gin.Context) {
	...
	if err != nil {
		app.Logger.Error("查询table失败!", zap.Error(err))
...
func (autoApi *AutoCodeApi) LLMAuto(c *gin.Context) {
	...
	params["mode"] = mode
	path := strings.ReplaceAll(app.Config.AutoCode.AiPath, "{FUNC}", "api/chat/ai")
	...
	if err != nil {
		app.Logger.Error("大模型生成失败!", zap.Error(err))
	...
	if resStruct.Code == 7 {
		app.Logger.Error("大模型生成失败!"+resStruct.Msg, zap.Error(err))
