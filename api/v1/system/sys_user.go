...
// @Summary  用户登录
// @Produce   application/json
// @Param    data  body      systemReq.Login                                             true  "邮箱, 密码"
// @Success  200   {object}  response.Response{data=systemRes.LoginResponse,msg=string}  "返回包括用户信息,token,过期时间"
// @Router   /base/emailLogin [post]
func (b *BaseApi) EmailLogin(c *gin.Context) {
	var l systemReq.EmailLogin
	err := c.ShouldBindJSON(&l)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	err = utils.Verify(l, utils.EmailLoginVerify)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	...
	if err != nil {
		global.Logger.Error("登陆失败! 邮箱不存在或者密码错误!", zap.Error(err))
		response.FailWithMessage("邮箱不存在或者密码错误", c)
		return
	}
	if user.Enable != 1 {
		global.Logger.Error("登陆失败! 用户被禁止登录!")
...
func (b *BaseApi) TokenNext(c *gin.Context, user system.SysUser) {
	j := &utils.JWT{SigningKey: []byte(global.ConfigServer.JWT.SigningKey)} // 唯一签名
	...
	if !global.ConfigServer.System.UseMultipoint {
...
// @Param    data  body      systemReq.Register                                            true  "用户名, 昵称, 密码, 角色ID"
// @Success  200   {object}  response.Response{data=systemRes.SysUserResponse,msg=string}  "用户注册账号,返回包括用户信息"
// @Router   /base/registerWithSmsCode [post]
func (b *BaseApi) RegisterWithSmsCode(c *gin.Context) {
	var r systemReq.RegisterWithSmsCode
	err := c.ShouldBindJSON(&r)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	...
	verifyUsernameWithSmsCode := userService.VerifyUsernameWithSmsCode(r.Username, r.SmsCode)
	if !verifyUsernameWithSmsCode {
		response.FailWithMessage("验证码错误", c)
		return
	}
...
func (b *BaseApi) GetUserList(c *gin.Context) {
	var pageInfo systemReq.SysUseSearch
