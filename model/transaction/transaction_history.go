...
type TransactionHistory struct {
	...
	SrcWalletId  *int   `json:"srcWalletId" form:"srcWalletId" gorm:"column:src_wallet_id;comment:与钱包表中的WalletID字段相关联，表示交易源钱包。如果钱包ID为0，则视为系统钱包;"`
	DestWalletId *int   `json:"destWalletId" form:"destWalletId" gorm:"column:dest_wallet_id;comment:与钱包表中的WalletID字段相关联，表示交易目标钱包。如果钱包ID为0，则视为系统钱包;"`
	...
	ProductId    *int   `json:"productId" form:"productId" gorm:"column:product_id;comment:与商品表中的ProductID字段相关联，表示交易涉及的商品。充值渠道只有兑换码，当为充值时，此ID为chat_ticket表的ID;"`
	Remark       *int   `json:"remark" form:"remark" gorm:"column:remark;comment:交易备注信息;"`
