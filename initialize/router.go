...
import (
	...
	"github.com/oldweipro/gin-admin/docs"
...
func Routers() *gin.Engine {
	...
	patrolRouter := router.RouterGroupApp.Patrol
	...
	Router.StaticFS(global.ConfigServer.Local.StorePath, http.Dir(global.ConfigServer.Local.StorePath)) // 为用户头像和文件提供静态地址
	// Router.Use(middleware.LoadTls())  // 如果需要使用https 请打开此中间件 然后前往 core/server.go 将启动模式 更变为 Router.RunTLS("端口","你的cre/pem文件","你的key文件")
	// 跨域，如需跨域可以打开下面的注释
	// Router.Use(middleware.Cors()) // 直接放行全部跨域请求
	// Router.Use(middleware.CorsByRules()) // 按照配置的规则放行跨域请求
	//global.Logger.Info("use middleware cors")
	docs.SwaggerInfo.BasePath = global.ConfigServer.System.RouterPrefix
	Router.GET(global.ConfigServer.System.RouterPrefix+"/swagger/*any", ginSwagger.WrapHandler(swaggerFiles.Handler))
	global.Logger.Info("register swagger handler")
	// 方便统一添加路由组前缀 多服务器上线使用

	PublicGroup := Router.Group(global.ConfigServer.System.RouterPrefix)
	{
		// 健康监测
	...
	PrivateGroup.Use(middleware.JWTAuth()).Use(middleware.CasbinHandler())
	{
		...
		patrolRouter.InitPatrolItemRouter(PrivateGroup)
		patrolRouter.InitPatrolTaskRouter(PrivateGroup)
		patrolRouter.InitPatrolSiteRouter(PrivateGroup)
		patrolRouter.InitIdentityRouter(PrivateGroup)
		patrolRouter.InitAccountRouter(PrivateGroup)
		patrolRouter.InitCertificationRecordRouter(PrivateGroup)
		patrolRouter.InitPersonnelRouter(PrivateGroup)
	}
	{
		ladderRouter := router.RouterGroupApp.Ladder
		ladderRouter.InitServerNodeRouter(PrivateGroup)
		ladderRouter.InitInboundsRouter(PrivateGroup)
	}
	{
	...
	}
	{
		...
		transactionRouter.InitChatTicketRouter(PrivateGroup)
