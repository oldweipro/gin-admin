...
func (m *mongo) Initialization() error {
	...
	if app.Config.Mongo.IsZap {
		opts = internal.Mongo.GetClientOptions()
	}
	ctx := context.Background()
	client, err := qmgo.Open(ctx, &qmgo.Config{
		Uri:              app.Config.Mongo.Uri(),
		Coll:             app.Config.Mongo.Coll,
		Database:         app.Config.Mongo.Database,
		MinPoolSize:      &app.Config.Mongo.MinPoolSize,
		MaxPoolSize:      &app.Config.Mongo.MaxPoolSize,
		SocketTimeoutMS:  &app.Config.Mongo.SocketTimeoutMs,
		ConnectTimeoutMS: &app.Config.Mongo.ConnectTimeoutMs,
		Auth: &qmgo.Credential{
			Username:   app.Config.Mongo.Username,
			Password:   app.Config.Mongo.Password,
			AuthSource: app.Config.Mongo.AuthSource,
		},
	}, opts...)
	if err != nil {
...
func (m *mongo) CreateIndexes(ctx context.Context, name string, indexes [][]string) error {
	collection, err := app.MongoClient.Database.Collection(name).CloneCollection()
	if err != nil {
	...
	for k1, v1 := range indexMap1 {
		...
		if len(fmt.Sprintf("%s.%s.$%s", collection.Name(), name, v1)) > 127 {
			err = app.MongoClient.Database.Collection(name).CreateOneIndex(ctx, options.IndexModel{
				Key:          v1,
