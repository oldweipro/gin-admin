...
func (a *autoCodeSqlite) GetDB(businessDB string) (data []response.Db, err error) {
	...
	if businessDB == "" {
		err = app.DBClient.Raw(sql).Find(&databaseList).Error
	} else {
		err = app.DbList[businessDB].Raw(sql).Find(&databaseList).Error
	}
	...
	// entities = append(entities, response.DBClient{global.Config.Sqlite.Dbname})
	return entities, err
}
...
func (a *autoCodeSqlite) GetTables(businessDB string, dbName string) (data []response.Table, err error) {
	...
	tabelNames := []string{}
	if businessDB == "" {
		err = app.DBClient.Raw(sql).Find(&tabelNames).Error
	} else {
		err = app.DbList[businessDB].Raw(sql).Find(&tabelNames).Error
	}
...
func (a *autoCodeSqlite) GetColumn(businessDB string, tableName string, dbName string) (data []response.Column, err error) {
	...
	if businessDB == "" {
		err = app.DBClient.Raw(sql).Scan(&columnInfos).Error
	} else {
		err = app.DbList[businessDB].Raw(sql).Scan(&columnInfos).Error
	}
