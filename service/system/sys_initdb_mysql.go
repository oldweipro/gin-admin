...
func (h MysqlInitHandler) WriteConfig(ctx context.Context) error {
	...
	global.ConfigServer.System.DbType = "mysql"
	global.ConfigServer.Mysql = c
	global.ConfigServer.JWT.SigningKey = uuid.NewString()
	cs := utils.StructToMap(global.ConfigServer)
	for k, v := range cs {
...
func (h MysqlInitHandler) EnsureDB(ctx context.Context, conf *request.InitDB) (next context.Context, err error) {
	...
	global.ConfigServer.AutoCode.Root, _ = filepath.Abs("..")
