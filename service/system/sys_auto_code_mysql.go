...
func (s *autoCodeMysql) GetTables(businessDB string, dbName string) (data []response.Table, err error) {
	...
	if businessDB == "" {
		err = app.DBClient.Raw(sql, dbName).Scan(&entities).Error
	} else {
		err = app.DbList[businessDB].Raw(sql, dbName).Scan(&entities).Error
	}
...
ORDER BY 
    c.ORDINAL_POSITION;`
	if businessDB == "" {
		err = app.DBClient.Raw(sql, tableName, dbName).Scan(&entities).Error
	} else {
		err = app.DbList[businessDB].Raw(sql, tableName, dbName).Scan(&entities).Error
	}
