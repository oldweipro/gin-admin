...
import (
	...
	"errors"
	"fmt"
	ast2 "github.com/oldweipro/gin-admin/utils/ast"
	"io"
	"mime/multipart"
	"os"
	"path/filepath"
	...
	"github.com/oldweipro/gin-admin/resource/autocode_template/subcontract"
...
func Init(Package string) {
	injectionPaths = []injectionMeta{
		{
			path: filepath.Join(global.ConfigServer.AutoCode.Root,
				global.ConfigServer.AutoCode.Server, fmt.Sprintf(global.ConfigServer.AutoCode.SApi, Package), "enter.go"),
			funcName:    "ApiGroup",
			structNameF: "%sApi",
		},
		{
			path: filepath.Join(global.ConfigServer.AutoCode.Root,
				global.ConfigServer.AutoCode.Server, fmt.Sprintf(global.ConfigServer.AutoCode.SRouter, Package), "enter.go"),
			funcName:    "RouterGroup",
			structNameF: "%sRouter",
		},
		{
			path: filepath.Join(global.ConfigServer.AutoCode.Root,
				global.ConfigServer.AutoCode.Server, fmt.Sprintf(global.ConfigServer.AutoCode.SService, Package), "enter.go"),
	...
	packageInjectionMap = map[string]astInjectionMeta{
		packageServiceName: {
			path: filepath.Join(global.ConfigServer.AutoCode.Root,
				global.ConfigServer.AutoCode.Server, "service", "enter.go"),
			importCodeF:  "github.com/oldweipro/gin-admin/%s/%s",
			packageNameF: "%s",
		...
		packageRouterName: {
			path: filepath.Join(global.ConfigServer.AutoCode.Root,
				global.ConfigServer.AutoCode.Server, "router", "enter.go"),
			importCodeF:  "github.com/oldweipro/gin-admin/%s/%s",
			packageNameF: "%s",
		...
		packageAPIName: {
			path: filepath.Join(global.ConfigServer.AutoCode.Root,
				global.ConfigServer.AutoCode.Server, "api/v1", "enter.go"),
			importCodeF:  "github.com/oldweipro/gin-admin/%s/%s",
			packageNameF: "%s",
			groupName:    "ApiGroup",
...
func (autoCodeService *AutoCodeService) CreateTemp(autoCode system.AutoCodeStruct, ids ...uint) (err error) {
	...
	if autoCode.AutoMoveFile { // 判断是否需要自动转移
...

		{
			// 在gorm.go 注入 自动迁移
			path := filepath.Join(global.ConfigServer.AutoCode.Root,
				global.ConfigServer.AutoCode.Server, global.ConfigServer.AutoCode.SInitialize, "gorm.go")
...

		{
			// router.go 注入 自动迁移
			path := filepath.Join(global.ConfigServer.AutoCode.Root,
				global.ConfigServer.AutoCode.Server, global.ConfigServer.AutoCode.SInitialize, "router.go")
...
func (autoCodeService *AutoCodeService) addAutoMoveFile(data *tplData) {
	...
	if strings.Contains(fileSlice[1], "server") {
		if strings.Contains(fileSlice[n-2], "router") {
			data.autoMoveFilePath = filepath.Join(global.ConfigServer.AutoCode.Root, global.ConfigServer.AutoCode.Server,
				fmt.Sprintf(global.ConfigServer.AutoCode.SRouter, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "api") {
			data.autoMoveFilePath = filepath.Join(global.ConfigServer.AutoCode.Root,
				global.ConfigServer.AutoCode.Server, fmt.Sprintf(global.ConfigServer.AutoCode.SApi, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "service") {
			data.autoMoveFilePath = filepath.Join(global.ConfigServer.AutoCode.Root,
				global.ConfigServer.AutoCode.Server, fmt.Sprintf(global.ConfigServer.AutoCode.SService, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "model") {
			data.autoMoveFilePath = filepath.Join(global.ConfigServer.AutoCode.Root,
				global.ConfigServer.AutoCode.Server, fmt.Sprintf(global.ConfigServer.AutoCode.SModel, data.autoPackage), base)
		} else if strings.Contains(fileSlice[n-2], "request") {
			data.autoMoveFilePath = filepath.Join(global.ConfigServer.AutoCode.Root,
				global.ConfigServer.AutoCode.Server, fmt.Sprintf(global.ConfigServer.AutoCode.SRequest, data.autoPackage), base)
		}
	} else if strings.Contains(fileSlice[1], "web") {
		if strings.Contains(fileSlice[n-1], "js") {
			data.autoMoveFilePath = filepath.Join(global.ConfigServer.AutoCode.Root,
				global.ConfigServer.AutoCode.Web, global.ConfigServer.AutoCode.WApi, base)
		} else if strings.Contains(fileSlice[n-2], "form") {
			data.autoMoveFilePath = filepath.Join(global.ConfigServer.AutoCode.Root,
				global.ConfigServer.AutoCode.Web, global.ConfigServer.AutoCode.WForm, filepath.Base(filepath.Dir(filepath.Dir(data.autoCodePath))), strings.TrimSuffix(base, filepath.Ext(base))+"Form.vue")
		} else if strings.Contains(fileSlice[n-2], "table") {
			data.autoMoveFilePath = filepath.Join(global.ConfigServer.AutoCode.Root,
				global.ConfigServer.AutoCode.Web, global.ConfigServer.AutoCode.WTable, filepath.Base(filepath.Dir(filepath.Dir(data.autoCodePath))), base)
		}
	}
}
...
func (autoCodeService *AutoCodeService) CreatePackageTemp(packageName string) error {
	...
	for i, s := range pendingTemp {
		pendingTemp[i].path = filepath.Join(global.ConfigServer.AutoCode.Root, global.ConfigServer.AutoCode.Server, filepath.Clean(fmt.Sprintf(s.path, packageName)))
	}
	// 选择模板
...
func (autoCodeService *AutoCodeService) CreatePlug(plug system.AutoPlugReq) error {
	...
	for _, tpl := range tplFileList {
		...
		if strings.Index(pathArr[2], "tpl") < 0 {
			dirPath := filepath.Join(global.ConfigServer.AutoCode.Root, global.ConfigServer.AutoCode.Server, fmt.Sprintf(global.ConfigServer.AutoCode.SPlug, plug.Snake+"/"+pathArr[2]))
			os.MkdirAll(dirPath, 0755)
		}
		file := filepath.Join(global.ConfigServer.AutoCode.Root, global.ConfigServer.AutoCode.Server, fmt.Sprintf(global.ConfigServer.AutoCode.SPlug, plug.Snake+"/"+tpl[len(plugPath):len(tpl)-4]))
...
func (autoCodeService *AutoCodeService) InstallPlugin(file *multipart.FileHeader) (web, server int, err error) {
	...
	if webIndex != -1 {
		err = installation(paths[webIndex], global.ConfigServer.AutoCode.Server, global.ConfigServer.AutoCode.Web)
		if err != nil {
	...
	if serverIndex != -1 {
		err = installation(paths[serverIndex], global.ConfigServer.AutoCode.Server, global.ConfigServer.AutoCode.Server)
	}
	return webIndex, serverIndex, err
}
...
func installation(path string, formPath string, toPath string) error {
	...
	var form = filepath.ToSlash(global.ConfigServer.AutoCode.Root + formPath + "/" + path)
	var to = filepath.ToSlash(global.ConfigServer.AutoCode.Root + toPath + "/plugin/")
...
func (autoCodeService *AutoCodeService) PubPlug(plugName string) (zipPath string, err error) {
	...
	webPath := filepath.Join(global.ConfigServer.AutoCode.Root, global.ConfigServer.AutoCode.Web, "plugin", plugName)
	serverPath := filepath.Join(global.ConfigServer.AutoCode.Root, global.ConfigServer.AutoCode.Server, "plugin", plugName)
	// 创建一个新的zip文件
	...
	return filepath.Join(global.ConfigServer.AutoCode.Root, global.ConfigServer.AutoCode.Server, fileName), nil
}
