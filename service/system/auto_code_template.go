...
func (s *autoCodeTemplate) checkPackage(Pkg string, template string) (err error) {
	switch template {
	case "package":
		apiEnter := filepath.Join(app.Config.AutoCode.Root, app.Config.AutoCode.Server, "api", "v1", Pkg, "enter.go")
		_, err = os.Stat(apiEnter)
		if err != nil {
			return fmt.Errorf("package结构异常,缺少api/v1/%s/enter.go", Pkg)
		}
		serviceEnter := filepath.Join(app.Config.AutoCode.Root, app.Config.AutoCode.Server, "service", Pkg, "enter.go")
		_, err = os.Stat(serviceEnter)
		if err != nil {
			return fmt.Errorf("package结构异常,缺少service/%s/enter.go", Pkg)
		}
		routerEnter := filepath.Join(app.Config.AutoCode.Root, app.Config.AutoCode.Server, "router", Pkg, "enter.go")
...
func (s *autoCodeTemplate) Create(ctx context.Context, info request.AutoCode) error {
	...
	err := app.DBClient.WithContext(ctx).Where("package_name = ?", info.Package).First(&autoPkg).Error
	if err != nil {
	...
	if info.AutoCreateApiToSql && !info.OnlyTemplate {
		apis := info.Apis()
		err := app.DBClient.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
			for _, v := range apis {
	...
	if info.AutoCreateMenuToSql {
		...
		var id uint
		err := app.DBClient.WithContext(ctx).First(&entity, "name = ?", info.Abbreviation).Error
		if err == nil {
			id = entity.ID
		} else {
			...
			err = app.DBClient.WithContext(ctx).Create(&entity).Error
			id = entity.ID
			if err != nil {
...
func (s *autoCodeTemplate) Preview(ctx context.Context, info request.AutoCode) (map[string]string, error) {
	...
	err := app.DBClient.WithContext(ctx).Where("package_name = ?", info.Package).First(&entity).Error
	if err != nil {
	...
	for key, writer := range codes {
		if len(key) > len(app.Config.AutoCode.Root) {
			key, _ = filepath.Rel(app.Config.AutoCode.Root, key)
		}
		// 获取key的后缀 取消.
...
func (s *autoCodeTemplate) generate(ctx context.Context, info request.AutoCode, entity model.SysAutoCodePackage) (map[string]strings.Builder, map[string]string, map[string]ast2.Ast, error) {
	...
	injections := make(map[string]ast2.Ast, len(asts))
	for key, value := range asts {
		keys := strings.Split(key, "=>")
		if len(keys) == 2 {
			if info.OnlyTemplate {
				if keys[1] == ast2.TypePackageInitializeGorm {
					continue
				}
			}
			if !info.AutoMigrate {
...
func (s *autoCodeTemplate) AddFunc(info request.AutoFunc) error {
	...
	err := app.DBClient.First(&autoPkg, "package_name = ?", info.Package).Error
	if err != nil {
		return err
	}
...
func (s *autoCodeTemplate) getTemplateStr(t string, info request.AutoFunc) (string, error) {
	tempPath := filepath.Join(app.Config.AutoCode.Root, app.Config.AutoCode.Server, "resource", "function", t+".tpl")
...
func (s *autoCodeTemplate) addTemplateToAst(t string, info request.AutoFunc) error {
	tPath := filepath.Join(app.Config.AutoCode.Root, app.Config.AutoCode.Server, "router", info.Package, info.HumpPackageName+".go")
	...
	if info.IsPlugin {
		tPath = filepath.Join(app.Config.AutoCode.Root, app.Config.AutoCode.Server, "plugin", info.Package, "router", info.HumpPackageName+".go")
	...
	funcDecl := ast2.FindFunction(astFile, funcName)
	stmtNode := ast2.CreateStmt(stmtStr)
...
func (s *autoCodeTemplate) addTemplateToFile(t string, info request.AutoFunc) error {
	...
	switch t {
	case "api.go":
		target = filepath.Join(app.Config.AutoCode.Root, app.Config.AutoCode.Server, "api", "v1", info.Package, info.HumpPackageName+".go")
	case "server.go":
		target = filepath.Join(app.Config.AutoCode.Root, app.Config.AutoCode.Server, "service", info.Package, info.HumpPackageName+".go")
	case "api.js":
		target = filepath.Join(app.Config.AutoCode.Root, app.Config.AutoCode.Web, "api", info.Package, info.PackageName+".js")
	}
	if info.IsPlugin {
		switch t {
		case "api.go":
			target = filepath.Join(app.Config.AutoCode.Root, app.Config.AutoCode.Server, "plugin", info.Package, "api", info.HumpPackageName+".go")
		case "server.go":
			target = filepath.Join(app.Config.AutoCode.Root, app.Config.AutoCode.Server, "plugin", info.Package, "service", info.HumpPackageName+".go")
		case "api.js":
			target = filepath.Join(app.Config.AutoCode.Root, app.Config.AutoCode.Web, "plugin", info.Package, "api", info.PackageName+".js")
		}
	}
