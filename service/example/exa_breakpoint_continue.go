...
func (e *FileUploadAndDownloadService) FindOrCreateFile(fileMd5 string, fileName string, chunkTotal int) (file example.ExaFile, err error) {
	...
	if errors.Is(app.DBClient.Where("file_md5 = ? AND is_finish = ?", fileMd5, true).First(&file).Error, gorm.ErrRecordNotFound) {
		err = app.DBClient.Where("file_md5 = ? AND file_name = ?", fileMd5, fileName).Preload("ExaFileChunk").FirstOrCreate(&file, cfile).Error
		return file, err
	}
	cfile.IsFinish = true
	cfile.FilePath = file.FilePath
	err = app.DBClient.Create(&cfile).Error
	return cfile, err
}
...
func (e *FileUploadAndDownloadService) CreateFileChunk(id uint, fileChunkPath string, fileChunkNumber int) error {
	...
	err := app.DBClient.Create(&chunk).Error
	return err
}
...
func (e *FileUploadAndDownloadService) DeleteFileChunk(fileMd5 string, filePath string) error {
	...
	err := app.DBClient.Where("file_md5 = ? ", fileMd5).First(&file).
	...
	err = app.DBClient.Where("exa_file_id = ?", file.ID).Delete(&chunks).Unscoped().Error
	return err
}
