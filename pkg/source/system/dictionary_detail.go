...
func (i *initDictDetail) InitializeData(ctx context.Context) (context.Context, error) {
	...
	dictList, ok := ctx.Value(initDict{}.InitializerName()).([]sysModel.SysDictionary)
	if !ok {
		return ctx, errors.Wrap(system.ErrMissingDependentContext,
			fmt.Sprintf("未找到 %s 表初始化数据", sysModel.SysDictionary{}.TableName()))
	}
	True := true
	dictList[0].SysDictionaryDetails = []sysModel.SysDictionaryDetail{
		{Label: "男", Value: "1", Status: &True, Sort: 1},
		{Label: "女", Value: "2", Status: &True, Sort: 2},
	}

	dictList[1].SysDictionaryDetails = []sysModel.SysDictionaryDetail{
	...
	dictList[2].SysDictionaryDetails = []sysModel.SysDictionaryDetail{
	...
	dictList[3].SysDictionaryDetails = []sysModel.SysDictionaryDetail{
	...
	dictList[4].SysDictionaryDetails = []sysModel.SysDictionaryDetail{
	...
	dictList[5].SysDictionaryDetails = []sysModel.SysDictionaryDetail{
