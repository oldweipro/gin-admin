...
func (l *Local) UploadBase64(base64Str, filename string) (string, string, error) {
	//TODO implement me
	panic("implement me")
}
...
func (*Local) UploadFile(file *multipart.FileHeader) (string, string, error) {
	...
	// 尝试创建此路径
	mkdirErr := os.MkdirAll(global.ConfigServer.Local.StorePath, os.ModePerm)
	if mkdirErr != nil {
		global.Logger.Error("function os.MkdirAll() Filed", zap.Any("err", mkdirErr.Error()))
		return "", "", errors.New("function os.MkdirAll() Filed, err:" + mkdirErr.Error())
	}
	// 拼接路径和文件名
	p := global.ConfigServer.Local.StorePath + "/" + filename
	filepath := global.ConfigServer.Local.Path + "/" + filename
...
func (*Local) DeleteFile(key string) error {
	p := global.ConfigServer.Local.StorePath + "/" + key
	if strings.Contains(p, global.ConfigServer.Local.StorePath) {
