...
func RollGormBack(pk, model string) {
	...
	// 如果多个 仅仅删除对应块即可
	// 如果单个 那么还需要剔除import
	path := filepath.Join(global.ConfigServer.AutoCode.Root, global.ConfigServer.AutoCode.Server, "initialize", "gorm.go")
			...
			if gen, ok := node.(*ast.GenDecl); ok {
				for i := range gen.Specs {
					if imspec, ok := gen.Specs[i].(*ast.ImportSpec); ok {
						if imspec.Path.Value == "\"github.com/oldweipro/gin-admin/model/"+pk+"\"" {
							gp = gen
							imI = i
							return false
						}
...
func RollRouterBack(pk, model string) {
	...
	path := filepath.Join(global.ConfigServer.AutoCode.Root, global.ConfigServer.AutoCode.Server, "initialize", "router.go")
	src, err := os.ReadFile(path)
	if err != nil {
		fmt.Println(err)
	}
